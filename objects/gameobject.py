...
class GameObject(pg.sprite.Sprite):
    ...
    def accelerate(self, direction) -> None:
        ...
        if self.speed > ship_max_speed: self.speed = ship_max_speed
        if self.speed < ship_min_speed: self.speed = ship_min_speed

    def spin(self, direction) -> None:
        ship_rotate_speed = 1
        self.state.heading += ship_rotate_speed * direction
        if self.state.heading >= 360: self.state.heading -= 360
        if self.state.heading < 0: self.state.heading += 360
...
class Obstacle(GameObject):
    def __init__(self, x_pos: int, y_pos: int, health: int, shield, destructible: bool) -> None:
        super().__init__(x_pos, y_pos, health, shield, 0, 0, 0, 0)  # obstacles have no direction, velocity nor heading
        self.destructible = destructible


class Projectile(GameObject):
    def __init__(self, x_pos: int, y_pos: int, health: int, shield: int, direction: int, velocity: int, heading: int, max_velocity:int, expiry_time: int) -> None:
        super().__init__(x_pos, y_pos, health, shield, direction, velocity, heading, max_velocity)

...
class SpriteSheet:
    ...
    def get_image(self, row, frame) -> pg.Surface:
...
class Animation:
    ...
    Reads single row from the sprite sheet and builds list of images which
    ...
    def __init__(
        ...
        self.sprites = []
        self.speed = (
            speed  # Effecticely the ms we wait for next animation frame - bigger means slower
        )
        ...
        self.frame_number = 0
        self.on_last_frame = (
            False  # gives way to check if animation is done (and ready to start over)
        )
        self.last_run = 0
    ...
    def get_image(self, repeat_delay=0) -> pg.Surface:
        # Returns the next image in the animation when active
        now = pg.time.get_ticks()
        time_since_last = now - self.last_run  # ticks since last run

        if (
            now > self.repeat_start + repeat_delay and self.active and time_since_last > self.speed
        ...
        try:
            ...
            logging.error(
                f"INDEX ERROR: unable to get frame (frame_number) {self.frame_number} from sprite sheet {self.ss}"
            )
            exit(1)
        return image
